...
use crate::{
    ...
    ffi::{block_t, builtin_exists, /* builtin_get_desc, */ builtin_get_names_ffi, parser_t},
    flog::{FLOG, FLOGF},
    function,
    ...
    wchar::prelude::*,
    wcstringutil::{
        string_fuzzy_match_string, string_prefixes_string_case_insensitive, CaseFold,
        StringFuzzyMatch,
    },
    wildcard::{wildcard_complete, wildcard_has, wildcard_match},
    wutil::{gettext::wgettext_impl_do_not_use_directly, wrealpath},
};
...
fn exec_subshell(
    ...
    if let Some(o) = outputs {
        *o = cmd_output.as_mut().unwrap().from_ffi();
    }
    retval
}
...
impl CompletionReceiver {
    ...
    pub fn subreceiver(&self) -> CompletionReceiver {
        CompletionReceiver::new(self.limit.saturating_sub(self.len()))
    }
}
...
static completion_autoloader: Lazy<Mutex<cxx::UniquePtr<ffi::autoload_t>>> =
    Lazy::new(|| Mutex::new(ffi::make_autoload_ffi(L!("fish_complete_path").to_ffi())));
...
impl<'ctx> Completer<'ctx> {
    ...
    pub fn perform_for_commandline(&mut self, cmdline: WString) {
        ...
        let _decrement = if let Some(parser) = self.ctx.parser.as_ref() {
            let mut p_guard = parser.lock().unwrap();
            let level = &mut p_guard.libdata_pod().complete_recursion_level;
            if *level >= 24 {
                FLOGF!(
                    error,
    ...
    fn complete_cmd_desc(&mut self, s: &wstr) {
        ...
        let keep_going = self
            .completions
            .iter()
            .any(|c| c.completion.is_empty() || c.completion.as_char_slice().last() != Some(&'/'));
        if !keep_going {
            return;
        }
    ...
    fn complete_cmd(&mut self, str_cmd: WString) {
        ...
        if str_cmd.is_empty() || (!str_cmd.contains('/') && str_cmd.as_char_slice()[0] != '~') {
            ...
            let possible_comp: Vec<Completion> = function::get_names(include_hidden)
                .into_iter()
                .map(Into::into)
                .collect();
            ...
            let possible_comp: Vec<Completion> = builtin_get_names_ffi()
                .from_ffi()
                .iter()
            ...
            self.complete_strings(
                &str_cmd,
                &{ Box::new(|name| ffi::builtin_get_desc_ffi(&name.to_ffi()).from_ffi()) },
                &possible_comp,
                CompleteFlags::empty(),
                ExpandFlags::empty(),
            );
    ...
    fn complete_abbr(&mut self, cmd: WString) {
        ...
        with_abbrs(|set| {
            for abbr in set.list() {
                if !abbr.is_regex() {
                    possible_comp.push(abbr.key.clone().into());
    ...
    fn complete_from_args(&mut self, s: &wstr, args: &wstr, desc: &wstr, flags: CompleteFlags) {
        ...
        let saved_state = if let Some(parser_ref) = self.ctx.parser.clone() {
            let mut parser = parser_ref.lock().unwrap();
            let saved_interactive = parser.libdata_pod().is_interactive;
            parser.libdata_pod().is_interactive = false;

            let status = parser.get_last_statuses().within_unique_ptr();
            Some((saved_interactive, status))
        } else {
            None
        };
        ...
        if let Some(parser) = self.ctx.parser.as_ref() {
            ...
            parser.libdata_pod().is_interactive = saved_interactive;

            parser.pin().set_last_statuses(status);
        }
    ...
    fn complete_param_for_command(
        ...
        // won't be using it.
        let cmd_exists = builtin_exists(&cmd.to_ffi())
        ...
        if !cmd_exists {
        ...
        } else if let Some(mut parser) = self.ctx.parser.as_ref().map(|p| p.lock().unwrap()) {
            ...
            .as_mut()
            .unwrap()
            .has_attempted_autoload(&cmd.to_ffi())
        {
        ...
        let all_options: Vec<Vec<CompleteEntryOpt>> = s_completion_map
            ...
            .filter_map(|(idx, completion)| {
                let m = if idx.is_path { &path } else { &cmd };
                if wildcard_match(m, &idx.name, false) {
        ...
        for options in all_options {
                        ...
                        for o in &options {
                            ...
                            if o.typ == CompleteOptionType::Short {
                                if let Some(prev_short_opt_pos) = prev_short_opt_pos {
                                    is_match = prev_short_opt_pos + 1 == popt.len()
                                        && o.option.char_at(0) == popt.char_at(prev_short_opt_pos);
                                }
                            } else if o.typ == CompleteOptionType::DoubleLong {
                                is_match = param_match(o, popt);
                            }
                            if is_match && self.conditions_test(&o.conditions) {
                                if o.result_mode.requires_param {
                                    use_common = false;
                                }
                                if o.result_mode.no_files {
            ...
            for o in &options {
                ...
                let is_match = whole_opt.starts_with(s);
                if !is_match {
    ...
    fn complete_param_expand(&mut self, s: &wstr, do_file: bool, handle_as_special_cd: bool) {
        ...
        let complete_from_start = sep_index.is_none() || !s.starts_with('-');
    ...
    fn complete_variable(&mut self, s: &wstr, start_offset: usize) -> bool {
        ...
        for env_name in self.ctx.vars.get_names(EnvMode::empty()) {
            ...
            let Some(m) = string_fuzzy_match_string(var, &env_name, anchor_start) else {
                continue;
            };

            let (comp, flags) = if !m.requires_full_replacement() {
    ...
    fn try_complete_user(&mut self, s: &wstr) -> bool {
        ...
        while let Some(pw_name) = getpwent_name() {
            ...
            if pw_name.starts_with(user_name) {
    ...
    fn apply_var_assignments<'a>(
        ...
        let Some(parser_ref) = self.ctx.parser.clone() else {
            return None;
        };
        let mut parser = parser_ref.lock().unwrap();
        ...
        let block = parser
            .pin()
            .push_block_ffi(block_t::variable_assignment_block().within_unique_ptr());
        ...
        Some(ScopeGuard::new((), move |_| {
            let p = parser_ref.clone();
            p.lock().unwrap().pin().pop_block(block);
        }))
    }
    ...
    fn complete_custom(&mut self, cmd: &wstr, cmdline: &wstr, ad: &mut CustomArgData) {
        ...
        let mut _remove_transient: Option<usize> = None;
        let wants_transient =
            (ad.wrap_depth > 0 || !ad.var_assignments.is_empty()) && !is_autosuggest;
        if wants_transient {
            todo!("need transient_commandlines {}", cmdline)
            /* let parser_ref = self.ctx.parser();
            parser_ref
                .libdata_mut()
    ...
    fn escape_opening_brackets(&mut self, argument: &wstr) {
        ...
        for comp in self.completions.iter_mut() {
...
fn short_option_pos(arg: &wstr, options: &[CompleteEntryOpt]) -> Option<usize> {
    ...
    for (pos, arg_char) in arg.chars().enumerate().skip(1) {
        ...
        if let Some(m) = m {
            if m.result_mode.requires_param {
                return Some(pos);
            }
        } else {
...
pub fn complete_load(cmd: &wstr, parser: &mut parser_t) -> bool {
    ...
    let path_to_load = completion_autoloader
        ...
        .unwrap()
        .resolve_command_ffi(
            &cmd.to_ffi(), /*, Environment::globals() &**EnvStack::globals()*/
        );
    if !path_to_load.is_null() {
        ffi::perform_autoload_ffi(&path_to_load, parser.pin());
        completion_autoloader
            .lock()
            .expect("mutex poisoned")
            .as_mut()
            .unwrap()
            .mark_autoload_finished(&cmd.to_ffi());
        loaded_new = true;
    }
    loaded_new
}
...
pub fn complete_invalidate_path() {
    ...
    let cmds = ffi::autoload_t::get_autoloaded_commands(completion_autoloader
        .lock()
        .expect("mutex poisoned")
        .as_mut()
        .unwrap());
