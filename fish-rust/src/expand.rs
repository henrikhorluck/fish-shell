...
use crate::ffi::history_session_id;
...
pub fn expand_string(
    ...
    let mut recv = CompletionReceiver::new(ctx.expansion_limit);
...
fn expand_variables(
    ...
    todo!("need history ffi, {}", is_single)
    /*    let mut history = None;
    let mut var = None;
    if var_name == L!("history") {
        history = Some(history_t::with_name(&history_session_id(vars)));
...
pub fn expand_cmdsubst(
    ...
    let mut sub_res: Vec<WString> = vec![];
    let job_group = ctx.job_group.clone();
    todo!("Need exec_subhell_for_expand");
    let subshell_status = 1; // exec_subshell_for_expand(subcmd, &mut ctx.parser(), job_group.as_ref(), &mut sub_res);
    if subshell_status != 0 {
        ...
        let err = match subshell_status {
            ...
            _ if subshell_status == STATUS_CMD_ERROR.unwrap() => {
                if ctx.parser.unwrap().lock().unwrap().is_eval_depth_exceeded() {
...
impl<'a, 'b, 'c> Expander<'a, 'b, 'c> {
    ...
    fn expand_string(
        ...
        if !flags.contains(ExpandFlags::FOR_COMPLETIONS) && expand_is_clean(&input) {
            if !out_completions.add(input.into()) {
        ...
        let mut completions: Vec<Completion> = vec![input.clone().into()];
    ...
    fn stage_wildcards(
        ...
        if has_wildcard && self.flags.contains(ExpandFlags::EXECUTABLES_ONLY) {
            ...
            if !self.flags.contains(ExpandFlags::FOR_COMPLETIONS) {
                if !out.add(path_to_expand.into()) {
                    return append_overflow_error(self.errors, None);
                }
            }
        }
...
crate::ffi_tests::add_test!("test_expand", || {
    ...
    fn expand_test(input: &wstr, flags: ExpandFlags, expected: Vec<WString>, failure_msg: &wstr) {
        ...
        let mut ctx = OperationContext::foreground(
            // .shared()
            // XXX: complete bogus
            unsafe { Box::pin(*parser_t::principal_parser_ffi()) },
            // &pwd, // todo!
            &no_cancel, // todo!
        ...
        ) == ExpandResultCode::error
        {
            if errors.is_empty() {
                ...
                panic!(
                    "{}",
                    errors[0].describe(input, ctx.parser.clone().unwrap().lock().unwrap().is_interactive())
                );
            }
        }
